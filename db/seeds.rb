...
User.create!(:name=>'나야나',:email => 'user2@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'개 짜증나는 은행')
#,:created_at=>'2016-11-01 17:22:32'
User.create!(:name=>'좋네요',:email => 'user3@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'진짜 빨리 망해라')
...
User.create!(:name=>'KB망해라',:email => 'user10@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'빨리 망했으면 좋겠습니다.')
...
User.create!(:name=>'나영수',:email => 'user14@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'서민한테는 대출 안해주는 은행')
...
User.create!(:name=>'지영이',:email => 'user18@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'사실 망한곳인데 세금으로 살린데지요')
#,:created_at=>'2016-12-22 23:12:35'
User.create!(:name=>'저도가입',:email => 'user19@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'대출 안되면 안된다하지, 왜 사람 기다리게 하는거야??')
#,:created_at=>'2016-12-26 22:21:11'
User.create!(:name=>'망해라',:email => 'user20@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'망하기만을 기도합니다.')
#,:created_at=>'2017-01-01 22:12:04'
User.create!(:name=>'정유한',:email => 'user21@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'하...  할 말을 잊었습니다....')
...
ReportCategory.create!(:id=>3,:title=>'강매')
ReportCategory.create!(:id=>4,:title=>'비리,횡령')
ReportCategory.create!(:id=>5,:title=>'횡포')
ReportCategory.create!(:id=>6,:title=>'기타')

Report.create!(:id=>1,:user_id=>1,:report_category_id=>1,:title=>'진짜 이놈들은 고객을 뭘로 보는건지?',:default_view_count=>156,:created_at=>'2016-08-02 18:49:49',:updated_at=>'2016-08-02 18:49:49',
content: '짜증 가득한 표정보면 정말 주먹이 우네요
...
content: '거의 10년 넘게 국민은행을 주거래 은행으로 사용했습니다.
...
Report.create!(:id=>5,:user_id=>31,:report_category_id=>5,:title=>'여권되는건데 왜 개고생시켜??',:default_view_count=>139,:created_at=>'2017-03-04 18:51:23',:updated_at=>'2017-03-04 18:51:23',
content: '날 더운날, 인터넷 뱅킹 비번 한계 이상으로 틀려서 풀려고 국민은행에 갔습니다.
마침 주민등록증이 없어서 여권을 가지고 갔습니다.
내 차례가 되자 이야기 하고 처리받으려고 하는데 여권으로 안된다고 하면서 말하는 띠꺼운 표정은 아직도 기억에 남네요.
여권 왜 안되냐고 하니, 씹듯이 대답안하고 주민등록증이나 등본으로 가져오라고 하네요.
여권으로 되는 걸로 알고 있는데  진짜 짜증나더라고요.
도로 집까지 땀 뻘뻘흘리며 와서 인터넷으로 등본띠어서 다시 땀 뻘뻘 흘리며 오니 정말 부들부들 분노가 폭팔하려고 하더라고요.
던지듯이 등본내고 처리받았는데 생각할수록 열받네요')

Report.create!(:id=>6,:user_id=>32,:report_category_id=>5,:title=>'참 어이가 없는 현금서비스 이율',:default_view_count=>139,:created_at=>'2017-04-04 18:51:23',:updated_at=>'2017-04-04 18:51:23',
content: '현금서비스라는게 이자가 비싸고 안 쓸수록 좋은건지는 모두알지만
...
Propose.create!(:id=>1,:user_id=>1,:title=>'ㅎㅎㅎ 뭐 제안안해도 알아서 망할듯 ㅋㅋㅋㅋㅋ',
                content:'이놈들 동시다발적으로 사고 친거 걸렸네요
요즘은 언론사에 먹일 돈이 딸리나? 예전부터 쭉 있던일인데 이제야 터지네요~~',:created_at=>'2014-08-04 18:51:23',:updated_at=>'2014-08-04 18:51:23')
Propose.create!(:id=>2,:user_id=>1,:title=>'당연한 이야기이지만 있는 계좌와 카드를 해지 하세요',
                content: '카드는 해지시켰었는데
계좌해지를 안하고 있었네요
...
Bank.create!(:id=>2,:title=>'경남은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>3,:title=>'광주은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>4,:title=>'기업은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>5,:title=>'농협',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>6,:title=>'대구은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>7,:title=>'도이치은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>8,:title=>'부산은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>9,:title=>'비엔피파리바은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>10,:title=>'산림조합',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>11,:title=>'산업은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>12,:title=>'상호저축은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>13,:title=>'새마을금고',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>14,:title=>'수출입은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>15,:title=>'수협중앙회',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>16,:title=>'신용협동조합',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>17,:title=>'신한은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>18,:title=>'외환은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>19,:title=>'우체국',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>20,:title=>'전북은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>21,:title=>'제주은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>22,:title=>'하나은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>23,:title=>'BOA(뱅크오브아메리카)',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>24,:title=>'HSBC',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>25,:title=>'JP모간 체이스',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>26,:title=>'SC은행',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>27,:title=>'우리투자증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>28,:title=>'교보증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>29,:title=>'대신증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>30,:title=>'대우증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>31,:title=>'동부증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>32,:title=>'동양증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>33,:title=>'메리츠증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>34,:title=>'미래에셋증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>35,:title=>'부국증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>36,:title=>'삼성증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>37,:title=>'신영증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>38,:title=>'신한금융투자',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>39,:title=>'아이엠투자증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>40,:title=>'유진투자증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>41,:title=>'이트레이드증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>42,:title=>'키움증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>43,:title=>'하나대투증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>44,:title=>'하이투자증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>45,:title=>'한국투자증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>46,:title=>'한화투자증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>47,:title=>'현대증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>48,:title=>'HMC투자증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>49,:title=>'LIG투자증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>50,:title=>'NH농협증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))
Bank.create!(:id=>51,:title=>'SK증권',:photo=>File.open(Rails.root.join("app", "assets", "images", "lovely_dog.jpg")))


ComplimentCategory.create!(:id=>1,:title=>'친절')
ComplimentCategory.create!(:id=>2,:title=>'대출승인')
ComplimentCategory.create!(:id=>3,:title=>'좋은상품 ')
ComplimentCategory.create!(:id=>4,:title=>'정직,성실')
ComplimentCategory.create!(:id=>5,:title=>'고객감동')
...
신한은행은 OTP만 만들면 리눅스도 인터넷 뱅킹이 잘 지원됩니다.')


FaqCategory.create!(:id=>1,:title=>'불친절')
FaqCategory.create!(:id=>2,:title=>'비리,횡령')
FaqCategory.create!(:id=>3,:title=>'강매')

Faq.create!(:id=>1,:faq_category_id=>1,:title=>'이 은행은 왜 이렇게 불친절 한거지요?',
:content=>'KB는 친절따위는 이미 쓰레기통에 버린지 오래인 은행이다.
어짜피 망해도 다시 정부에서 지원해줄 꺼니깐 우리는 걱정이 없어! ㅎㅎ 부럽냐?')
Faq.create!(:id=>2,:faq_category_id=>1,:title=>'서민들에게 대출 해주긴 하나요?',
:content=>'귀찮은 거지들 대출해주기는 정말 짜증나
그래서 우리는 사채업자들에게 대출을 잘해준다고
거지들은 어짜피 사채쓰면 되잖어?')
Faq.create!(:id=>3,:faq_category_id=>1,:title=>'대체 고객을 뭘로 보는거지요?',:content=>'귀찮은 거지새끼들로 보는데?')
Faq.create!(:id=>4,:faq_category_id=>1,:title=>'저런 직원들은 어디서 구했나요?',:content=>'당연히 아무것도 모르는 대학교 졸업한 애송이들로 채워넣지
광고비에 돈을 펑펑 쓰고 있기떄문에 직접 당해보지 않으면 여기가 좋은곳인줄 알거든')
Faq.create!(:id=>5,:faq_category_id=>1,:title=>'짜증이 얼굴에 쓰여있는 이유는 뭐지요?',:content=>'일 끝나고 놀아야되는데 거지들이 귀찮게 하면 얼마나 짜증나는데!!!')
Faq.create!(:id=>6,:faq_category_id=>2,:title=>'왜 이 살인자-은행은 비리의 온상인거지요?',:content=>'우리는 각 임직원이 은행돈도 내돈, 세금도 우리돈이라는 생각으로 근무하며
돈을 물쓰듯 합니다. 몇억쯤 띠어 먹는거는 우리의 특권입니다.')
Faq.create!(:id=>7,:faq_category_id=>2,:title=>'대체 언제까지 그런식으로 비리와 횡령을 해나갈껀가요?',:content=>'아 어짜피 거지들이 모은돈좀 쓰겠다는 시비거냐??, 아님 나라에서 세금으로 해줄꺼고
우리의 띠어먹는 이 실력으로 세계로 진출해서 한국의 이름을 세계에 알릴꺼다.')
Faq.create!(:id=>8,:faq_category_id=>3,:title=>'왜 강매를 시키나요?',:content=>'우리는 망해도 세금으로 먹여살려주는 대형 은행이다.
꼬우면 다른은행가~')
...
Notice.create!(id: 2,user_id: 1,title: 'www.antikb.site 도메인 추가 및 재오픈',:created_at=>'2017-01-04 18:51:23',:updated_at=>'2017-01-04 18:51:23',
content: '개인적인 운영비용 충당의 어려움으로 인해 또다시 얼마동안 서비스되지 못하고 폐쇄되었던점 사과드립니다.
...
Notice.create!(id: 3,user_id: 1,title: 'www.anti-kb.site 도메인 추가 및 재오픈.',:created_at=>'2020-10-14 18:51:23',:updated_at=>'2020-10-14 18:51:23',
