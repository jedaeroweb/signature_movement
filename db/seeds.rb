...
User.create!(:name=>'잠자는-사자',:email => 'admin@jedaeroweb.co.kr',:password => 'a12345', :password_confirmation => 'a12345',:description=>'살인자 은행 망해라!!',admin: true,user_pictures_attributes: [{ picture: File.open(Rails.root.join("public", "images", "lovely_dog.jpg")) }])
...
Article.create!(:title=>'이새끼들 하는짓은 언제나 이런짓',:url=>'https://www.youtube.com/watch?v=h42e5Er2uMc',:description=>'어휴..... 욕도 안나온다',:created_at=>15.day.ago)
Article.create!(:title=>'KB쓰다가는 대규모 손실날수도 있어요',:url=>'https://www.youtube.com/watch?v=tKmB__zHJPg',:description=>'언제나 망하는 시스템 만드는 KB로 증권거래한다고요??!!',:created_at=>10.day.ago)
Article.create!(:title=>'이런 기사는 나오지도 않네??',:url=>'https://www.newstopkorea.com/news/articleView.html?idxno=40205',:description=>'언제나 돈 날려먹는 KB은행, 이런 사고들 국내 언론에 잘 안나와요',:created_at=>5.day.ago)
...
Report.create!(:id=>7,:user_id=>14,:report_category_id=>5,:title=>'이놈들은 앱을 사용자 열받게 하려고 만든게 틀림없음',:default_view_count=>139,:created_at=>'2024-08-12 18:51:23',:updated_at=>'2024-08-12 18:51:23',
content: '앱만드는 걸로 장난치나??? 진짜
대체 앱이 몇개 인거??
이 앱으로 뭐 할라 그려면 저거 다운 받으라고 하고
그거 다운 받아서 뭐 할라그럼 또 다른거 다운 받아야 된데...  미친~!!!
앱 만드는걸로 뭐 또 횡령 하는 건 수가 있는건가??
아니면 사용자 열받는거 즐기려고 이렇게 하는건지??
정말 미친 은행이 아닐 수 없다
')

Report.create!(:id=>8,:user_id=>14,:report_category_id=>5,:title=>'예가람저축은행??',:default_view_count=>139,:created_at=>'2026-03-02 18:51:23',:updated_at=>'2026-03-02 18:51:23',
               content: '별 그지 같은곳 다 보겠네
대출 될 것 처럼 이것저것 다 요구하고 존니 귀찬게 하더니
입금내역도 보내줬더니 봐도 잘 모르겠다고??, 좀 쳐 봐!!!
더하기 빼기도 못하냐??  그냥 더해도 수입 100만원 훨씬 넘는데 ㅂㅅ 같은 놈들
니네도 안봐도 비디오다.  어서 망해라
')
...
Bank.create!(:id=>1,:title=>'우리은행')
Bank.create!(:id=>2,:title=>'경남은행')
Bank.create!(:id=>3,:title=>'광주은행')
Bank.create!(:id=>4,:title=>'기업은행')
Bank.create!(:id=>5,:title=>'농협')
Bank.create!(:id=>6,:title=>'대구은행')
Bank.create!(:id=>7,:title=>'도이치은행')
Bank.create!(:id=>8,:title=>'부산은행')
Bank.create!(:id=>9,:title=>'비엔피파리바은행')
Bank.create!(:id=>10,:title=>'산림조합')
Bank.create!(:id=>11,:title=>'산업은행')
Bank.create!(:id=>12,:title=>'상호저축은행')
Bank.create!(:id=>13,:title=>'새마을금고')
Bank.create!(:id=>14,:title=>'수출입은행')
Bank.create!(:id=>15,:title=>'수협중앙회')
Bank.create!(:id=>16,:title=>'신용협동조합')
Bank.create!(:id=>17,:title=>'신한은행')
Bank.create!(:id=>18,:title=>'외환은행')
Bank.create!(:id=>19,:title=>'우체국')
Bank.create!(:id=>20,:title=>'전북은행')
Bank.create!(:id=>21,:title=>'제주은행')
Bank.create!(:id=>22,:title=>'하나은행')
Bank.create!(:id=>23,:title=>'BOA(뱅크오브아메리카)')
Bank.create!(:id=>24,:title=>'HSBC')
Bank.create!(:id=>25,:title=>'JP모간 체이스')
Bank.create!(:id=>26,:title=>'SC은행')
Bank.create!(:id=>27,:title=>'우리투자증권')
Bank.create!(:id=>28,:title=>'교보증권')
Bank.create!(:id=>29,:title=>'대신증권')
Bank.create!(:id=>30,:title=>'대우증권')
Bank.create!(:id=>31,:title=>'동부증권')
Bank.create!(:id=>32,:title=>'동양증권')
Bank.create!(:id=>33,:title=>'메리츠증권')
Bank.create!(:id=>34,:title=>'미래에셋증권')
Bank.create!(:id=>35,:title=>'부국증권')
Bank.create!(:id=>36,:title=>'삼성증권')
Bank.create!(:id=>37,:title=>'신영증권')
Bank.create!(:id=>38,:title=>'신한금융투자')
Bank.create!(:id=>39,:title=>'아이엠투자증권')
Bank.create!(:id=>40,:title=>'유진투자증권')
Bank.create!(:id=>41,:title=>'이트레이드증권')
Bank.create!(:id=>42,:title=>'키움증권')
Bank.create!(:id=>43,:title=>'하나대투증권')
Bank.create!(:id=>44,:title=>'하이투자증권')
Bank.create!(:id=>45,:title=>'한국투자증권')
Bank.create!(:id=>46,:title=>'한화투자증권')
Bank.create!(:id=>47,:title=>'현대증권')
Bank.create!(:id=>48,:title=>'HMC투자증권')
Bank.create!(:id=>49,:title=>'LIG투자증권')
Bank.create!(:id=>50,:title=>'NH농협증권')
Bank.create!(:id=>51,:title=>'SK증권')
...
Compliment.create!(:id=>1,:user_id=>1,:bank_id=>1,:compliment_category_id=>1,:title=>'우리은행 좋네요',:default_view_count=>137,:created_at=>'2023-08-02 18:49:49',:updated_at=>'2023-08-02 18:49:49',
content: '역시 망할 살인자-은행에 대적할 은행은 우리은행이지요~
홈페이지 진짜 잘만들었네
ㅂㅅ같은 살인자 은행놈들은 언제 플래시 버릴려나?')

Compliment.create!(:id=>2,:user_id=>1,:bank_id=>17,:compliment_category_id=>1,:title=>'신한은행도 좋음',:default_view_count=>131,:created_at=>'2022-09-11 14:33:10',:updated_at=>'2022-09-11 14:33:10',
...
Compliment.create!(:id=>3,:user_id=>1,:bank_id=>26,:compliment_category_id=>1,:title=>'SC은행 어떻게 된건가?',:default_view_count=>137,:created_at=>'2025-08-02 18:49:49',:updated_at=>'2025-08-02 18:49:49',
content: 'SC은행 친절하고 좋았는데, 요즘은 지점이 별로 안 보이네요
어떻게 망해야될 KB는 바글바글 늘어나고
좋은 은행들은 없어지는 모르겠네??
우간다보다 못한 금융 만드는데 KB가 일등 공신이지요
')

Compliment.create!(:id=>4,:user_id=>1,:bank_id=>26,:compliment_category_id=>1,:title=>'OK저축은행',:default_view_count=>137,:created_at=>'2026-03-02 18:49:49',:updated_at=>'2026-03-02 18:49:49',
                   content: '저축은행은 대표 OK저축은행, 지금은 완전한 제도권으로 서민들의 힘이 되어주는 저축은행입니다.
치열했던 전국시대를 통일한것 같은 이미지이지요
통일한 그 능력있는 시스템이 검증된것이라고 할 수 있겠습니다.
')
